...
import { formatCurrency, formatPerMile, formatNumber, formatPercent } from '@/components/calculators/shared/formatters';
...
export default function InteractiveCalculator() {
  ...
  useEffect(() => {
    if (results) {
      const target = Math.abs(savingsPercentage) * 100;
      const step = target / 50;
      let current = 0;
      const interval = setInterval(() => {
        current += step;
        if (current >= target) {
          setSavingsProgress(target);
          clearInterval(interval);
        } else {
          setSavingsProgress(current);
        }
      }, 20);
      return () => clearInterval(interval);
    }
  }, [savingsPercentage, results]);
  ...
  // Prepare cost split data
  const costSplitData = results ? [
    { name: 'Upfront', diesel: results.diesel.netUpfrontCost, bev: results.bev.netUpfrontCost },
    { name: 'Operating', diesel: results.diesel.annualOperatingCost * 10, bev: results.bev.annualOperatingCost * 10 }
  ] : [];
